...
class HuggingfaceRepository(HFStyleRepository):
    ...
    def _fetch_safetensors_metadata(self):
        ...
        for file_info in files:
            ...
            if fnmatch.fnmatch(path, '*.safetensors'):
                self.safetensors_files.append(
                    {
                        'filename': path,
                        'oid': file_info.get('oid', ''),  # This is the git blob sha
                    }
                )
    ...
    def _fetch_checksum_for_file(self, filename):
        """Fetch checksum for a specific file from HuggingFace API."""
        if fnmatch.fnmatch(filename, "*.json"):
            return ""
        # Construct full repo path (organization/name)
        repo_path = f"{self.organization}/{self.name}"
