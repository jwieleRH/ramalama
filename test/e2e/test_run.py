...
@pytest.mark.parametrize(
    ...
    [
        # fmt: off
        pytest.param(
            [], f".*{TEST_MODEL_FULL_NAME}.*", None, None, True, None, id="check test_model", marks=skip_if_no_container
        ),
        pytest.param(
            [],
            r".*--cache-reuse 256",
            ...
            True,
            None,
            id="check cache-reuse is being set",
        ...
        pytest.param(
            ...
            False,
            None,
            id="check ctx-size is not show by default",
            marks=skip_if_no_container,
        ),
        pytest.param(
            [],
            ...
            False,
            None,
            id="check --seed is not set by default",
            marks=skip_if_no_container,
        ),
        pytest.param(
            [],
            ...
            False,
            PIPE,
            id="check -t -i is not present without tty",
            marks=skip_if_no_container,
        ),
        pytest.param(
            ["--env", "a=b", "--env", "test=success", "--name", "foobar"],
            r"--env a=b --env test=success",
            None,
            None,
            True,
            None,
        ...
        pytest.param(
            ...
            True,
            None,
            id="check --oci-runtime",
            marks=skip_if_no_container,
        ),
        pytest.param(
            ["--net", "bridge", "--name", "foobar"],
            r".*--network bridge",
            None,
            {"RAMALAMA_CONFIG": "NUL" if platform.system() == "Windows" else '/dev/null'},
            True,
            None,
            id="check --net=bridge with RAMALAMA_CONFIG=/dev/null",
            marks=skip_if_no_container,
        ),
        pytest.param(
            ["--name", "foobar"],
            f".*{TEST_MODEL_FULL_NAME}.*",
            None,
            {"RAMALAMA_CONFIG": "NUL" if platform.system() == "Windows" else '/dev/null'},
            True,
            None,
            id="check test_model with RAMALAMA_CONFIG=/dev/null",
            marks=skip_if_no_container,
        ),
        pytest.param(
            ["-c", "4096", "--name", "foobar"],
            ...
            True,
            None,
            id="check --ctx-size 4096 with RAMALAMA_CONFIG=/dev/null",
            marks=skip_if_no_container,
        ),
        pytest.param(
            ["--cache-reuse", "512", "--name", "foobar"],
            r".*--cache-reuse 512",
            None,
            {"RAMALAMA_CONFIG": "NUL" if platform.system() == "Windows" else '/dev/null'},
            True,
            None,
            id="check --cache-reuse with RAMALAMA_CONFIG=/dev/null",
            marks=skip_if_no_container,
        ),
        pytest.param(
            ["--name", "foobar"],
            ...
            True,
            None,
            id="check --temp default value is 0.8 with RAMALAMA_CONFIG=/dev/null",
            marks=skip_if_no_container,
        ),
        pytest.param(
            ["--seed", "9876", "--name", "foobar"],
            r".*--seed 9876",
            None,
            {"RAMALAMA_CONFIG": "NUL" if platform.system() == "Windows" else '/dev/null'},
            True,
            None,
            id="check --seed 9876 with RAMALAMA_CONFIG=/dev/null",
            marks=skip_if_no_container,
        ),
        pytest.param(
            ["--name", "foobar"],
        ...
        pytest.param(
            ["--pull", "never", "-c", "4096", "--name", "foobbar"],
            r".*--pull never",
            None,
            None,
            True,
            None,
            id="check never pull policy",
            marks=skip_if_no_container,
        ),
        pytest.param(
            [],
            ...
            True,
            None,
            id="check pull policy with RAMALAMA_CONFIG",
            marks=skip_if_no_container,
        ),
        pytest.param(
            ["--name", "foobar"],
        ...
        pytest.param(
            ...
            r".*foo:latest.*serve",
            None,
            {"RAMALAMA_IMAGE": "foo:latest"},
            True,
            None,
            id="check run with RAMALAMA_IMAGE=foo:latest",
            marks=skip_if_no_container,
        ),
        pytest.param(
            ["--ctx-size", "4096"],
            r".*serve.*--ctx-size 4096",
            None,
            None,
            True,
            None,
            id="check --ctx-size 4096",
        ...
        pytest.param(
            ...
            True,
            None,
            id="check --cache-reuse is set by default to 256",
            marks=skip_if_container,
        ),
        pytest.param(
            [],
            r".*-e ASAHI_VISIBLE_DEVICES=99",
            None,
            {"ASAHI_VISIBLE_DEVICES": "99"},
            True,
            None,
            id="check ASAHI_VISIBLE_DEVICES env var",
            marks=skip_if_no_container,
        ),
        pytest.param(
            [],
            r".*-e CUDA_LAUNCH_BLOCKING=1",
            None,
            {"CUDA_LAUNCH_BLOCKING": "1"},
            True,
            None,
            id="check CUDA_LAUNCH_BLOCKING env var",
            marks=skip_if_no_container,
        ),
        pytest.param(
            [],
            r".*-e HIP_VISIBLE_DEVICES=99",
            None,
            {"HIP_VISIBLE_DEVICES": "99"},
            True,
            None,
            id="check HIP_VISIBLE_DEVICES env var",
            marks=skip_if_no_container,
        ),
        pytest.param(
            [],
            r".*-e HSA_OVERRIDE_GFX_VERSION=0.0.0",
            None,
            {"HSA_OVERRIDE_GFX_VERSION": "0.0.0"},
            True,
            None,
            id="check HSA_OVERRIDE_GFX_VERSION env var",
            marks=skip_if_no_container,
        ),
        pytest.param(
            [],
            ...
            None,
            {"HIP_VISIBLE_DEVICES": "99", "HSA_OVERRIDE_GFX_VERSION": "0.0.0"},
            True,
            None,
            id="check HIP_VISIBLE_DEVICES & HSA_OVERRIDE_GFX_VERSION env vars",
            marks=skip_if_no_container,
        ),
        pytest.param(
            ["--device", "NUL" if platform.system() == "Windows" else '/dev/null', "--pull", "never"],
            r".*--device (NUL|/dev/null) .*",
            None,
            None,
            True,
            None,
            id="check --device=/dev/null",
            marks=skip_if_no_container,
        ),
        pytest.param(
            ["--device", "none", "--pull", "never"],
            r".*--device.*",
            None,
            None,
            False,
            None,
            id="check --device with unsupported value",
            marks=skip_if_no_container,
        ),
        # fmt: on
    ],
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            ...
            22,
            True,
            id="raise error on conflict between nocontainer and --name",
            marks=skip_if_container,
        ),
        pytest.param(
            ["--privileged"],
            ...
            22,
            True,
            id="raise error on conflict between nocontainer and --privileged",
            marks=skip_if_container,
        ),
        # fmt: on
    ],
